...
import { logs, paginateArray } from "../utils/index.js";
import fs from "fs";
import {
  createFolderAtCLM,
  uploadDocuments,
...
export const processUnitaryFiles = async (
  ...
  try {
    ...
    const documentPath = `./src/Files/${mainDirectory}/${directory}/${file}`;
    logs(
      "requestError.txt",
 ...
 * @param {string} directory - Subdirectory within the main directory.
 * @param {string} token - Authentication token.
 * @example await processFullFolder('Files', 'clientes', 'token123');
 */
export const processFullFolder = async (mainDirectory, directory, token) => {
  const documentPath = `./src/${mainDirectory}/${directory}/`;
  ...
  for (const folder of folders) {
    ...
    const folderID = await createFolderAtCLM(folder, folderPath, token, fatherFolderID);
